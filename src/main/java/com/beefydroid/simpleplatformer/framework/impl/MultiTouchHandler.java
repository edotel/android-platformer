...
import com.beefydroid.simpleplatformer.framework.Input.TouchEvent;
    ...
    public boolean onTouch(View v, MotionEvent event){
        synchronized(this){
            ...
            for (int i = 0; i < MAX_TOUCHPOINTS; i++) {
                //If number of points is lower, then points must have been un-touched
                if (i >= pointerCount) {
                    isTouched[i] = false ;
                    id[i] = -1;
                    continue ;
                ...
                if(event.getAction() != MotionEvent.ACTION_MOVE && i != pointerIndex){
                    //If it's and up/down/cancel/out event, mask the id to see id we should
                    //process point
                    continue;
                }
                switch (action){
